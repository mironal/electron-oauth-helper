...
class OAuth2Provider {
  ...
  constructor(config) {
    ...
    this.customAccessTokenRequestHeaders = {}
  ...
  /**
   * Sets the custom headers for  Access Token Request.
   *
   * This can be used to include your own headers in your request
   * that are not included in the OAuth2 specification.

   * @example
   * ```js
   *  const provider = new OAuth2Provider(config)
   *    .withCustomAccessTokenRequestHeaders({Authorization: "Bearer XXX"})
   * ```
   *
   * @param {object} param
   */
  withCustomAccessTokenRequestHeaders(headers) {
    this.customAccessTokenRequestHeaders = headers
    return this
  }
  ...
  perform(window) {
    ...
    debug("start OAUTH2")
    return task(config, window, this.customAuthorizationRequestParameter, this.customAccessTokenRequestParameter, this.customAccessTokenRequestHeaders)
      .then(resp => {
        this.finished = true
