...
test("Authorization Code Grant: request success with custom parameter", async t => {
  ...
  const provider = new OAuth2Provider({
    ...
    .withCustomAuthorizationRequestParameter({ custom_auth: "custom auth param" })
    .withCustomAccessTokenRequestParameter({ custom_token: "custom token param" })
...
test("Authorization Code Grant: authorize request failure", async t => {
  ...
  const resp = await t.throws(provider.perform(window))

  t.deepEqual({
    error: "access_denied"
  }, resp.query)
  t.is(resp.message, "error response")
})
...
test("Implicit Grant: request success", async t => {
  ...
  routing(koa)(router => {
    ...
    router.get("/authorize", ctx => {
      ...
      const redirectURL = `${ctx.query.redirect_uri}#access_token=abcd&token_type=bearer`
...
test("Implicit Grant: request success with custom parameter", async t => {
  ...
  const provider = new OAuth2Provider({
    ...
    client_id: "client id",
    redirect_uri: `http://localhost:${port}/redirect`
  }).withCustomAuthorizationRequestParameter({ hoge: "⁽⁽◝( ˙ ꒳ ˙ )◜⁾⁾" })
